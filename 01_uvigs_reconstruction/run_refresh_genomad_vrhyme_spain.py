...
CHECKV_DB = Path("/mnt/nvme/Taxonomia/Proteins/Checkv/DB_CLEAN/checkv-db-v1.5")
# USER: set your base path here
GENOMAD_ROOT = Path("/mnt/nvme2/Metagenome_Spain_Miuvigs")
# USER: set your base path here
FASTQ_ROOT = Path("/mnt/m2/Metagenomes")
# USER: set your base path here
OUTPUT_ROOT = Path("/mnt/nvme2/Metagenome_Spain_Miuvigs/Refresh_Genomad")
RCLONE_REMOTE_ROOT = "ubu:/home/vMags/Spain"
# USER: set your base path here
...
PHABOX_INPUT_DIR = OUTPUT_ROOT / "Spain_phabox_inputs"
PHABOX_OUTPUT_DIR = OUTPUT_ROOT / "Spain_phabox_outputs"
PHAGE_QC_LOG = OUTPUT_ROOT / "Spain_phage_qc.log"
...
def discover_samples() -> list[str]:
    samples: list[str] = []
    for path in sorted(GENOMAD_ROOT.glob("SRR*/Annotation/_sample_level/genomad_selected_mediumplus")):
...
def build_filtered_tables(sample: str, contamination_cutoff: float, keep_low_quality: bool) -> tuple[Path, int]:
    ...
    seqs = read_fasta(paths["virus_fna"])
    ...
    write_fasta(filtered_fasta, seqs, keep_ids)
...
def run_vrhyme(sample: str, filtered_fasta: Path, threads: int) -> None:
    ...
    fastq1 = FASTQ_ROOT / f"{sample}_1.fastq"
    fastq2 = FASTQ_ROOT / f"{sample}_2.fastq"

    if not fastq1.exists() or not fastq2.exists():
        raise FileNotFoundError(f"{sample}: FASTQs not found in {FASTQ_ROOT}")
    ...
    attempts: list[int] = []
    for attempt_threads in [threads, 4, 2, 1]:
    ...
    if not (checkv_bins / "quality_summary.tsv").exists():
        ...
        for attempt_threads in attempts:
            try:
                ...
                if (checkv_bins / "tmp").exists():
                    shutil.rmtree(checkv_bins / "tmp", ignore_errors=True)
        if last_exc is not None:
            raise last_exc
...
def write_master_bin_taxonomy() -> Path:
    master_path = OUTPUT_ROOT / "Spain_vRhyme_bin_taxonomy.tsv"
    rows: list[dict[str, str]] = []
    fieldnames: list[str] | None = None

    for sample_table in sorted(OUTPUT_ROOT.glob("SRR*/bin_taxonomy.tsv")):
...
def build_spain_phabox_inputs() -> tuple[Path, Path, Path]:
    PHABOX_INPUT_DIR.mkdir(parents=True, exist_ok=True)
    linked_fasta = PHABOX_INPUT_DIR / "Spain_vRhyme_bins.linked.prefixed.fasta"
    protein_faa = PHABOX_INPUT_DIR / "Spain_vRhyme_bins.linked.prefixed.faa"
    mapping_tsv = PHABOX_INPUT_DIR / "Spain_vRhyme_bin_accession_map.tsv"
...
def finalize_spain_phage_qc(phabox_threads: int) -> dict[str, Path]:
    master_base = write_master_bin_taxonomy()
    linked_fasta, protein_faa, mapping_tsv = build_spain_phabox_inputs()
    ...
    for quality_path in sorted(OUTPUT_ROOT.glob("SRR*/postbin_checkv/checkv_bins/quality_summary.tsv")):
    ...
    master_qc = OUTPUT_ROOT / "Spain_vRhyme_phage_qc_master.tsv"
    primary_tsv = OUTPUT_ROOT / "Spain_vRhyme_phage_primary_mediumplus.tsv"
    supplementary_all_tsv = OUTPUT_ROOT / "Spain_vRhyme_phage_supplementary_all.tsv"
    supplementary_only_tsv = OUTPUT_ROOT / "Spain_vRhyme_phage_supplementary_only.tsv"
    exclusion_tsv = OUTPUT_ROOT / "Spain_vRhyme_phage_exclusion_audit.tsv"
...
def run_vamb_smoketest(sample: str, subset_fasta: Path, threads: int) -> None:
    ...
    bam = vrhyme_subset / "vRhyme_bam_files" / f"{sample}_1.sorted.bam"
    ...
    if not (vrhyme_subset / "vRhyme_best_bins.0.summary.tsv").exists():
        run(
            [
                ...
                str(subset_fasta),
                "-r",
                str(FASTQ_ROOT / f"{sample}_1.fastq"),
                str(FASTQ_ROOT / f"{sample}_2.fastq"),
                "-o",
                str(vrhyme_subset),
                "-t",
                str(threads),
...
def main() -> int:
    ...
    parser.add_argument("--samples", nargs="*", help="SRR sample IDs. Default: auto-discover directly from ubu:/home/vMags/Spain.")
    ...
    if args.finalize_phage_qc_only:
        outputs = finalize_spain_phage_qc(phabox_threads=args.phabox_threads)
        for label, path in outputs.items():
            print(f"{label}\t{path}")
        return 0

    samples = args.samples or discover_remote_samples()
    if not samples:
